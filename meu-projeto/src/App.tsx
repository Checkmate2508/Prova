...
import { bookingsOnDate, cancelBooking, endTime, formatDate, formatDuration, formatTime, getStatus, INITIAL_COURTS, localDate, parseStart, sortBookings, STATUS, validateBooking } from './domain'
...
function loadInitialData() {
  try { return { data: readData(), error: '' } } catch {
    return { data: { courts: INITIAL_COURTS, bookings: [] } as AppData, error: 'Não foi possível ler o armazenamento. As alterações ficarão apenas nesta sessão; os dados anteriores não serão sobrescritos.' }
  }
}
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <div className="workspace">
      <header className="topbar"><div className="breadcrumb">Centro esportivo <span>/</span><strong>{page === 'agenda' ? 'Agendamentos' : 'Quadras'}</strong></div><span className="local-indicator"><span />{storageError ? 'Sessão temporária' : 'Armazenamento local'}</span></header>
      <main>
        ...
        {page === 'agenda' ? <>
          ...
          <section className="agenda-panel" aria-labelledby="agenda-title">
            ...
            {visibleBookings.length > 0 ? <div className="table-scroll"><table><thead><tr><th scope="col">QUADRA / MODALIDADE</th><th scope="col">HORÁRIO</th><th scope="col">DURAÇÃO</th><th scope="col">SITUAÇÃO</th><th scope="col"><span className="sr-only">Ações</span></th></tr></thead><tbody>{visibleBookings.map((booking) => {
              ...
              return <tr key={booking.id} className={status === 'cancelled' ? 'cancelled-row' : ''}><td><div className="court-cell"><span className={`court-avatar sport-${court.sport === 'Tênis' ? 'tennis' : court.sport === 'Vôlei' ? 'volley' : 'futsal'}`}><Icon name="court" size={21} /></span><div><strong>{court.name}</strong><span>{court.sport} <span className="middot">·</span> {court.floor}</span><span className="customer-name">{booking.customer}</span></div></div></td><td><strong className="time-range">{formatTime(booking.startsAt)} <span>–</span> {formatTime(end)}{crossesDay && <small> +1 dia</small>}</strong><span className="cell-secondary">{new Date(booking.startsAt).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' })}</span></td><td><span className="duration-tag"><Icon name="clock" size={14} />{formatDuration(booking.durationMinutes)}</span></td><td><span className={`status-badge ${status}`}><span />{STATUS[status].label}</span>{booking.cancellation && <details className="cancel-details"><summary>Ver motivo</summary><p>{booking.cancellation.reason}</p><small>Cancelado em {formatDate(booking.cancellation.cancelledAt)} às {formatTime(booking.cancellation.cancelledAt)}</small></details>}</td><td className="action-cell">{(status === 'scheduled' || status === 'ongoing') ? <button className="cancel-action" onClick={() => setModal({ type: 'cancel', bookingId: booking.id })} aria-label={`Cancelar reserva de ${booking.customer} em ${court.name} às ${formatTime(booking.startsAt)}`}>Cancelar</button> : <span className="no-action">—</span>}</td></tr>
